...
import { Plus, RefreshCw, Loader2, X, Image as ImageIcon, ArrowUp, ArrowDown, Star, Upload, Play } from 'lucide-react'
...
export default function AdminGaleriaPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <input ref={loteInputRef} type="file" accept="image/*,video/*" multiple className="hidden" onChange={handleUploadLote} />

      {/* FILTRO POR EVENTO */}
      <div className="flex gap-2 flex-wrap">
        {['todos', ...eventos.map(e => e.id), 'sem_evento', 'destaques'].map(val => {
          const label = val === 'todos' ? 'Todas'
            : val === 'sem_evento' ? 'Sem evento'
            : val === 'destaques' ? 'Destaques'
            : eventos.find(e => e.id === val)?.nome ?? val
          return (
            <button key={val} onClick={() => setEventoFiltro(val)}
              className={`px-3 py-1.5 rounded-sm border text-xs font-accent tracking-wider uppercase transition-all
                ${eventoFiltro === val ? 'border-dourado bg-dourado/10 text-dourado' : 'border-white/10 text-bege-escuro/60 hover:border-dourado/30'}`}>
              {label}
            </button>
          )
        })}
...
const MidiaModal: React.FC<{
  ...
  const salvar = async () => {
    if (!form.url) { toast.error('URL da mídia é obrigatória'); return }
    setSalvando(true)
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0f0c07] border border-dourado/30 rounded-sm"
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="admin-label">URL da foto ou vídeo *</label>
            <input
            ...
            />
            <p className="text-[10px] text-bege-escuro/30 mt-1">
              Cole URL do Cloudinary (foto ou vídeo .mp4), caminho em /public, ou URL externa
            </p>
          </div>

  ...
  const handleUploadLote: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    try {
      for (const file of files) {
        // Detectar orientação pelo tamanho real da imagem
        const orientacao = await new Promise<'portrait' | 'landscape'>((resolve) => {
          ...
          img.onerror = () => { resolve('landscape'); URL.revokeObjectURL(url) }
          img.src = url
        })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-white/8">
        <button onClick={() => setAba('galeria')}
          className={`flex items-center gap-2 px-4 py-2.5 text-xs font-accent tracking-wider uppercase border-b-2 transition-all -mb-px ${aba === 'galeria' ? 'border-dourado text-dourado' : 'border-transparent text-bege-escuro/50 hover:text-bege-escuro/80'}`}>
          <Grid size={13} /> Todas as Mídias
        </button>
        <button onClick={() => setAba('home')}
          className={`flex items-center gap-2 px-4 py-2.5 text-xs font-accent tracking-wider uppercase border-b-2 transition-all -mb-px ${aba === 'home' ? 'border-dourado text-dourado' : 'border-transparent text-bege-escuro/50 hover:text-bege-escuro/80'}`}>
              ...
              <div className="admin-card">
                <div className="mb-5">
                  ...
                  <p className="font-body text-xs text-bege-escuro/50">
                    Slots 1-3 são verticais (portrait), slots 4-7 são horizontais (landscape).
                  </p>
                </div>
                <div className="space-y-3">
                  <p className="font-accent text-[9px] tracking-[0.25em] uppercase text-bege-escuro/35">
                    Linha 1 — Cards Verticais (portrait)
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {slotsHome.slice(0, 3).map(s => (
                      <SlotCard key={s.slot} slot={s} selecionado={slotSelecionado === s.slot}
                        onSelecionar={() => setSlotSelecionado(slotSelecionado === s.slot ? null : s.slot)}
                        onLimpar={() => atribuirSlot(s.slot, null)} alturaPreview="h-44" />
                    ))}
                  </div>
                  <p className="font-accent text-[9px] tracking-[0.25em] uppercase text-bege-escuro/35 mt-4">
                    Linha 2 — Cards Horizontais (landscape)
                  </p>
                  <div className="grid grid-cols-4 gap-3">
                    {slotsHome.slice(3, 7).map(s => (
                      <SlotCard key={s.slot} slot={s} selecionado={slotSelecionado === s.slot}
                        onSelecionar={() => setSlotSelecionado(slotSelecionado === s.slot ? null : s.slot)}
                        onLimpar={() => atribuirSlot(s.slot, null)} alturaPreview="h-24" />
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h4 className="font-accent text-xs tracking-widest uppercase text-dourado">
                        Slot {slotSelecionado} — {SLOT_LABELS[slotSelecionado]}
                      </h4>
                      <p className="font-body text-[11px] text-bege-escuro/45 mt-0.5">
                        Recomendado: {SLOT_HINT[slotSelecionado]}
                      </p>
                    </div>
                  ...
                  {fotos.length === 0 ? (
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2 max-h-80 overflow-y-auto pr-1">
                      {fotos.map(foto => {
                        ...
                        return (
                          <button key={foto.id} onClick={() => atribuirSlot(slotSelecionado, foto.id)}
                            className={`relative aspect-square rounded-sm overflow-hidden border-2 transition-all duration-200 ${jaNesseSlot ? 'border-dourado ring-1 ring-dourado/40' : 'border-white/10 hover:border-dourado/60'}`}
                            title={foto.alt ?? foto.url}>
                            {ehVideo ? (
                              <video src={foto.url} muted loop playsInline autoPlay className="absolute inset-0 w-full h-full object-cover" />
                            ) : (
                              <Image src={foto.url} alt={foto.alt ?? ''} fill className="object-cover" sizes="120px" />
                            )}
                            {jaNesseSlot && (
                              <div className="absolute inset-0 bg-dourado/20 flex items-center justify-center">
                                <div className="bg-dourado rounded-full p-1"><Check size={12} className="text-preto-profundo" strokeWidth={3} /></div>
                              </div>
                            )}
                            ...
                            {ehVideo && (
                              <div className="absolute top-1 left-1 bg-black/70 border border-dourado/20 px-1 py-0.5 rounded-sm">
                                <Play size={7} className="text-dourado fill-dourado" />
                              </div>
                            )}
                            {/* Badge orientação */}
                            <div className={`absolute bottom-0 right-0 text-[7px] font-accent px-1 py-0.5 ${foto.orientacao === 'portrait' ? 'bg-purple-900/70 text-purple-200' : 'bg-blue-900/70 text-blue-200'}`}>
                              {foto.orientacao === 'portrait' ? '9:16' : '16:9'}
                            </div>
                          </button>
                        )
                      })}
                    ...
                    return (
                      <div key={foto.id} className="admin-card p-2 group relative">
                        <div className="relative aspect-square rounded-sm overflow-hidden bg-black/40 mb-2">
                          ...
                          {/* Badge orientação */}
                          <div className={`absolute top-2 right-2 text-[8px] font-accent px-1.5 py-0.5 rounded-sm ${foto.orientacao === 'portrait' ? 'bg-purple-900/80 text-purple-200' : 'bg-blue-900/80 text-blue-200'}`}>
                            {foto.orientacao === 'portrait' ? '9:16' : '16:9'}
                          </div>
                          {slotAtribuido && (
                        ...
                        <p className="font-body text-[10px] text-bege-escuro/25 px-1">
                          #{foto.ordem + 1}{ehVideo ? ' · vídeo' : ''} · {foto.orientacao}{slotAtribuido ? ` · Home S${slotAtribuido.slot}` : ''}
                        </p>
                      </div>
                    )
                  })}
...
// ─── CARD DE SLOT ─────────────────────────────────────────────
function SlotCard({ slot, selecionado, onSelecionar, onLimpar, alturaPreview }: {
  slot: SlotHome; selecionado: boolean; onSelecionar: () => void; onLimpar: () => void; alturaPreview: string
}) {
  const midia = slot.galeria
  const ehVideo = midia ? isVideo(midia.url) : false
  return (
    <div className={`rounded-sm border-2 overflow-hidden transition-all duration-200 ${selecionado ? 'border-dourado shadow-[0_0_12px_rgba(201,168,76,0.25)]' : 'border-white/10 hover:border-dourado/40'}`}>
      <div className={`relative ${alturaPreview} bg-black/40 cursor-pointer group`} onClick={onSelecionar}>
        {midia ? (
          <>
            {ehVideo ? (
              <video src={midia.url} autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover" />
            ) : (
              <Image src={midia.url} alt={midia.alt ?? ''} fill className="object-cover" sizes="200px" />
            )}
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
              <span className="opacity-0 group-hover:opacity-100 transition-opacity font-accent text-[10px] tracking-widest uppercase text-white bg-black/60 px-2 py-1 rounded-sm">Trocar</span>
            </div>
          </>
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 group-hover:bg-dourado/5 transition-colors">
            ...
            <span className="font-accent text-[9px] tracking-[0.2em] uppercase text-bege-escuro/30 group-hover:text-bege-escuro/60 transition-colors">Clique para definir</span>
          </div>
        )}
      </div>
      <div className="px-2 py-2 bg-black/20 flex items-center justify-between gap-1">
        <div className="min-w-0">
          <p className="font-accent text-[9px] tracking-[0.15em] uppercase text-dourado/60 leading-none">Slot {slot.slot}</p>
          <p className="font-body text-[10px] text-bege-escuro/40 mt-0.5 truncate">{midia ? (midia.alt || 'Sem descrição') : SLOT_HINT[slot.slot]}</p>
        </div>
        {midia && (
          <button onClick={e => { e.stopPropagation(); onLimpar() }} className="flex-shrink-0 p-1 text-bege-escuro/30 hover:text-red-400 transition-colors" title="Remover mídia deste slot">
            <X size={12} />
          </button>
        )}
      </div>
...
const MidiaModal: React.FC<{
  foto: FotoGaleria | null; eventos: EventoOpt[]; onClose: () => void; onSalvo: () => void
  ...
  const [form, setForm] = useState({
    ...
    ordem: foto?.ordem ?? 0,
    orientacao: (foto?.orientacao ?? 'landscape') as 'portrait' | 'landscape',
  })
  ...
  // Detectar orientação de imagem a partir de URL (para modo URL)
  const detectarOrientacaoUrl = (url: string) => {
    if (!url) return
    if (isVideo(url)) {
      // Vídeos verticais do Cloudinary geralmente são portrait
      setForm(p => ({ ...p, orientacao: 'portrait' }))
      return
    }
    setDetectando(true)
    const img = new window.Image()
    img.onload = () => {
      setForm(p => ({ ...p, orientacao: img.naturalHeight > img.naturalWidth ? 'portrait' : 'landscape' }))
      setDetectando(false)
    }
    img.onerror = () => setDetectando(false)
    img.src = url
  }
  ...
  const handleFileChange: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    // Detectar orientação antes de subir
    const orientacao = await new Promise<'portrait' | 'landscape'>((resolve) => {
      const img = document.createElement('img')
      const objectUrl = URL.createObjectURL(file)
      img.onload = () => { resolve(img.naturalHeight > img.naturalWidth ? 'portrait' : 'landscape'); URL.revokeObjectURL(objectUrl) }
      img.onerror = () => { resolve('landscape'); URL.revokeObjectURL(objectUrl) }
      img.src = objectUrl
    })
    ...
    try {
      ...
      setForm(p => ({ ...p, url, alt: p.alt || file.name.replace(/\.[^/.]+$/, ''), orientacao }))
      setPreviewUrl(url)
      toast.success(`Foto carregada · ${orientacao === 'portrait' ? 'Vertical 9:16' : 'Horizontal 16:9'}`)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0f0c07] border border-dourado/30 rounded-sm max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
            ...
            <div>
              ...
              <button type="button" onClick={() => fileInputRef.current?.click()} disabled={enviandoUpload}
                ...
                {enviandoUpload ? (
                  ...
                  <><div className="w-12 h-12 border border-dourado/20 rounded-sm flex items-center justify-center"><FolderOpen size={22} className="text-dourado/50" /></div>
                  <div className="text-center"><p className="font-body text-sm text-bege-escuro/70">Clique para selecionar uma foto</p><p className="font-body text-xs text-bege-escuro/35 mt-1">JPG, PNG, WEBP · Máx. 10MB</p></div></>
                )}
              </button>
            </div>
          )}
            ...
            <div>
              ...
              <input className="admin-input font-mono text-xs" value={form.url}
                onChange={e => {
                  ...
                  setForm(p => ({ ...p, url: val }))
                  setPreviewUrl(val)
                  if (val.length > 20) detectarOrientacaoUrl(val)
                }}
                placeholder="https://res.cloudinary.com/.../video.mp4" autoFocus />
              <p className="text-[10px] text-bege-escuro/30 mt-1.5">Orientação detectada automaticamente</p>
              {previewUrl && (
          ...
          {/* Orientação — detectada automaticamente, mas editável */}
          <div>
            <label className="admin-label">
              Orientação
              {detectando && <span className="ml-2 text-bege-escuro/40 text-[10px]">detectando...</span>}
            </label>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setForm(p => ({ ...p, orientacao: 'portrait' }))}
                className={`py-2.5 rounded-sm text-xs font-accent tracking-[0.15em] uppercase border transition-all ${form.orientacao === 'portrait' ? 'border-purple-400/60 bg-purple-900/20 text-purple-300' : 'border-white/10 text-bege-escuro/50 hover:border-white/20'}`}>
                Vertical 9:16
              </button>
              <button type="button" onClick={() => setForm(p => ({ ...p, orientacao: 'landscape' }))}
                className={`py-2.5 rounded-sm text-xs font-accent tracking-[0.15em] uppercase border transition-all ${form.orientacao === 'landscape' ? 'border-blue-400/60 bg-blue-900/20 text-blue-300' : 'border-white/10 text-bege-escuro/50 hover:border-white/20'}`}>
                Horizontal 16:9
              </button>
            </div>
            <p className="text-[10px] text-bege-escuro/30 mt-1">Detectada automaticamente · ajuste se necessário</p>
          </div>
